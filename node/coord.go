...
const (
	...
	msgPing            coordMsgType = 11 // liveness probe for signer selection
)
...
func checkSignerSet(members map[tss.PartyID]bool, groupThreshold int, curve Curve, signers []tss.PartyID) error {
	...
	// The floor is scheme-dependent: FROST is T-of-N, but the ECDSA
	// construction needs 2T-1 (requiredSigners explains why). Checking only
	// against T would let an under-sized ECDSA set through to the KMS, which
	// rejects it — but rejecting it here keeps the boundary check honest and
	// the error attributable to the initiator.
	required := requiredSigners(curve, groupThreshold)
	if len(distinct) < required {
		return fmt.Errorf("signer set below %s minimum: %d < %d",
			curve, len(distinct), required)
	}
	return nil
}
...
func (n *Node) handleCoordStream(s libp2pnet.Stream) {
	...
	//
	// Answered only for a group this node and the caller both belong to, so
	// the probe cannot be used by an arbitrary peer to enumerate node state.
	// libp2p has already authenticated the remote peer, so the membership
	// check is meaningful.
	//
	// The ACK deliberately round-trips through the key manager rather than
	// returning immediately. A node whose KMS socket is dead still accepts
	// libp2p streams and would pass a transport-level check, then fail to
	// produce a share once a real session started. GetKeyInfo on a key that
	// does not exist returns (nil, nil), so any sentinel key ID works as a
	// reachability probe and none needs to exist.
	if msg.Type == msgPing {
		caller := tss.PartyID(s.Conn().RemotePeer().String())
		n.groupsMu.RLock()
		grp := n.groups[msg.GroupID]
		n.groupsMu.RUnlock()
		if grp == nil || !tss.NewPartyIDSlice(grp.Members).Contains(caller) {
			s.Write([]byte{coordNACK})
			return
		}
		if _, err := n.km.GetKeyInfo(msg.GroupID, pingProbeKeyID, CurveSecp256k1); err != nil {
			n.log.Warn("coord: ping failed, key manager unreachable", zap.Error(err))
			s.Write([]byte{coordNACK})
			return
		}
		s.Write([]byte{coordACK})
	...
	case msgSign:
		...
		signCurve := Curve(msg.Curve)
		if signCurve == "" {
			signCurve = CurveSecp256k1
		}
		if err := checkSignerSet(members, threshold, signCurve, msg.Signers); err != nil {
...
func (n *Node) broadcastCoord(ctx context.Context, targets []tss.PartyID, msg coordMsg) error {
	...
	for _, pid := range peers {
		...
		if !ok {
			// Feed the miss through the same channel as every other outcome
			// rather than returning here. An early return would hand the
			// caller a bare error instead of *coordBroadcastError, so the
			// retry in runThresholdSign would not recognise it and would not
			// route around this party; the liveness tracker would never learn
			// the party is unreachable; and results from goroutines already
			// spawned for earlier parties would be dropped unread.
			ch <- result{pid, fmt.Errorf("no connected peer for party %s", pid)}
			continue
		}
		go func() {
	...
	// Drain every result rather than returning on the first error. Two
	// reasons: the caller needs the full set of failed parties to pick a
	// different signer set on retry, and each outcome is a liveness signal
	// worth recording. Returning early would discard both.
	var failed []tss.PartyID
	var firstErr error
	for range peers {
		r := <-ch
		if r.err != nil {
			failed = append(failed, r.party)
			if firstErr == nil {
				firstErr = r.err
			}
			if n.liveness != nil {
				n.liveness.RecordFailure(r.party)
			}
			n.log.Debug("coord: party unreachable",
				zap.String("party", string(r.party)), zap.Error(r.err))
			continue
		}
		if n.liveness != nil {
			n.liveness.RecordSuccess(r.party, time.Since(start))
		}
		n.log.Debug("coord: party ready", zap.String("party", string(r.party)))
	}

	if len(failed) > 0 {
		return &coordBroadcastError{Failed: tss.NewPartyIDSlice(failed), Err: firstErr}
	}
	return nil
}

// coordBroadcastError reports which parties did not accept a coord message.
// The sign path uses Failed to exclude those parties from the next attempt.
type coordBroadcastError struct {
	Failed tss.PartyIDSlice
	Err    error
}

func (e *coordBroadcastError) Error() string {
	return fmt.Sprintf("%d parties unreachable %v (first error: %v)",
		len(e.Failed), tss.PartyIDsToStrings(e.Failed), e.Err)
}

func (e *coordBroadcastError) Unwrap() error { return e.Err }

// pingProbeKeyID is the key ID used by liveness probes. It is never created;
// GetKeyInfo returns (nil, nil) for a missing key, so the call proves the key
// manager answered without depending on any particular key existing.
const pingProbeKeyID = "__signet_liveness_probe__"

// sendPing opens a coord stream to pid and waits for the ACK. It returns nil
// only when the peer answered affirmatively, meaning its signetd is processing
// coord messages and its key manager responded.
func (n *Node) sendPing(ctx context.Context, pid tss.PartyID, groupID string) error {
	peerID, ok := n.host.PeerForParty(pid)
	if !ok {
		return fmt.Errorf("no peer mapping for party %s", pid)
	}
	payload, err := cbor.Marshal(coordMsg{Type: msgPing, GroupID: groupID})
	if err != nil {
		return fmt.Errorf("marshal ping: %w", err)
	}
	st, err := n.host.LibP2PHost().NewStream(ctx, peerID, coordProtocol)
	if err != nil {
	...
	if _, err := io.ReadFull(st, ack); err != nil {
		return fmt.Errorf("ping ack: %w", err)
	}
	if ack[0] == coordNACK {
		return fmt.Errorf("ping rejected by %s", pid)
	}
	return nil
}
